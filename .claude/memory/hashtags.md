...
## Status
Done, backend and frontend (Phase 20 backend 2026-08-06; frontend 2026-08-07 alongside the Phase 18 Compete tab — see [[challenges]]'s "Phase 18 + 20 frontend" section for the full cross-feature picture). 17 backend tests passing against real Postgres (`test_open_challenges.py`, shared with the open-challenge coverage). Migration `f4a7b2c9d813` applied to dev. Frontend `tsc`/`expo lint`/`expo export --platform web` clean (24 routes, including `/tag/[slug]`).
...
- `GET /hashtags/{slug}` → `HashtagOut` = `{id, slug, display_text, meme_count, challenge_id}`. `404` unknown.
- `GET /hashtags/{slug}/memes?cursor=&limit=` → `FeedPage` — the tag feed, the discovery surface that makes an open challenge spread.
