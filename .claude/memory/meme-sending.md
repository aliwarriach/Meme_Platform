...
- **`send_meme` must resolve the meme via `services/memes.py::get_visible_meme` (visibility-checked), never a raw `db.get(Meme, id)`** — fixed during the Phase 16 security review, see Gotchas.
...
## Gotchas
- **IDOR fixed in Phase 16's security review**: `send_meme` originally loaded the target meme via `db.get(Meme, data.meme_id)` — an unchecked raw lookup, meaning a user who merely *knew or guessed* a meme's UUID (e.g. a Friends-only post by a non-friend, or a private-community post by a non-member) could still forward it to one of their own friends via meme-sending, completely bypassing the audience system, even though they could never see it in any feed. Fixed by resolving through `services/memes.py::get_visible_meme(db, current_user, meme_id)` instead, which applies the same `meme_visibility_clause` the feed uses and 404s if the sender can't actually see the meme. Regression test: `test_cannot_send_a_meme_you_cannot_see` in `test_meme_sending.py`.
