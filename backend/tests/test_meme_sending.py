...
def test_websocket_delivers_a_shimmed_send_in_real_time():
    with TestClient(app) as test_client:
        ...
        # Alice is the one who calls `/meme-sending/send`, which opens a *new*
        # conversation with Bob the first time — that requires a verified email
        # (SecurityFeatures.md F-1); Bob doesn't need to be verified for this test.
        asyncio.run(mark_email_verified(alice["user"]["id"]))
