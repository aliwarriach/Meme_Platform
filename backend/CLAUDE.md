...
- **Post audience**: a post's audience is a join table (`post_audience`), not a single enum column, but audience is only ever *client-chosen* for personal posts (`POST /memes`, Friends and/or Public — never a client-supplied `community`). Community posts (`POST /communities/{id}/memes`) are only created from inside a community and get their audience rows **derived server-side** from that community's `privacy`: always a `community` row, plus a `public` row too if the community is `open` — never accept a client-supplied community audience/ID on the generic endpoint. Every feed/read query filters by the requester's actual access (friendship accepted, community membership) — never trust a client-supplied audience filter as the sole gate.
